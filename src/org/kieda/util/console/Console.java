package org.kieda.util.console;
...
/**
 * A really nice console that is built into the AbstractCore.
 * todo - make a separate class that has print functions
 * so the client isn't forced to use Console to get fancy printing.
 * 
 * @author kieda
 */
        ...
        switch(envi){
            case LEFT_COLUMN_ENVI:{
                ...
                snow:for(int h = 0; h < buf.length; h++){
                    ...
                    if(f.length>0 && f[0]=="\n"){
                        if(f.length!=2)  break snow;
                        if(f[1] == null) break snow;

                        //dummy variable, we make a line of the maximum length 
                        //in the cols
                        
                        int len = 0;
                        for(int i = 0; i < f.length; i++){len+= spacing + colWid.get(i);}
                        if(len==0) break snow;
                        char[] cit = f[1].toCharArray();
                        //the chars that will be run through and printed
            ...
            case MIDDLE_COLUMN_ENVI:{
                ...
                int max_length = spacing;//spacing/2 + (exSp?1:0);//LHS spacing
                
                for(Object ss: buf){//go through all of the lines
                    String[] f = (String[]) ss;
                    if(f.length==0 || !f[0].equals("\n"))
                    for(int i = 0; i < f.length; i++){//go through the columns
                        int colI = f[i].length();//the column at i
                        if(i>=colWid.size()){//shouldn't ever be greater, but 
                                             //whatev
                ...
                for(int i = 0; i < colWid.size(); i++){
                    max_length += colWid.get(i) + spacing;
                }
                snow:for(int h = 0; h < buf.length; h++){
                    String[] f = (String[]) buf[h];
                    if(f.length>0 && f[0]=="\n"){
...
//            System.arraycopy(addition.get(0), l+1, sail, l+1, sail.length - (l+1));
    ...
    /**
     * fill the line in an environment with the given chars
     */
    public static void fillel(String chars){
        try{
            if(chars.contains("\n")) throw new InvalidInputException("Input can't contain \"\\n\", as you cant fill a line with it.");
            chars = chars.replaceAll("\t", "    ");//to prevent any tabbing problems
//            buffer.add(new String[]{"\n", chars});//\n is the dummy
            
            ArrayList<String[]> addition = new ArrayList<String[]>(1);
            addition.add(new String[]{"\n",chars});
            String[] long_line = new String[]{"\n",chars};
//            for(String l : lines){//goes through list of new lines
//                if(regex!=null)
//                    addition.add(breakpoint(l, regex));
//                else {
...
//                }
//            }
            //in the case that the buffer has not been written to yet.  
            if(buffer.isEmpty()) buffer.add(long_line);
            else 
//                if(!addition.isEmpty())   //we shouldn't need this.
                {
                ...
                //      1. if the last line in the buffer is empty, add the line to the previous line.
                //      2. if the previous line is not empty, add the line to a new line.
                
                int last_buffer_index = buffer.size()-1;//the last index of the buffer
                if((buffer.get(last_buffer_index).length == 1 && buffer.get(last_buffer_index)[0].isEmpty())||
                   buffer.get(last_buffer_index).length == 0 //this case should not happen, but just in case
                        ){
                    buffer.set(last_buffer_index, long_line); 
                    //over-write the previous line with the "long line"
                } else{
                    //add the long line to the end, after the previously printed material.
                    buffer.add(long_line);
                }
            }
            //the new line at the end of the buffer.
            buffer.add(new String[]{""});
        } catch(Exception e){}
    }
    ...
    protected Console(){};
    /**
     * defaults - help, exit, and pause.
     *
     * the client may add more by creating new instances of module,
...
//                if(params.length==0){//sets the AbstractCore to running if it was suspended, and to suspended if was not
//                    eventQ.add(new EVT() { @Override public void event() {
//                        if(AbstractCore.state == AbstractCore.CoreState.SUSPENDED)
//                            AbstractCore.setCoreState(AbstractCore.CoreState.RUNNING);
//                        else
//                            AbstractCore.setCoreState(AbstractCore.CoreState.SUSPENDED);
//                    }});
//                } else {
...
//        };
    ...
    public void run(){
        l:while(true){
            if(!building){
                System.out.print("["+AbstractCore.project_name()+"] : ");
